...
function People() {
    ...
    const renderSection = (title, members, isBig = false) => (
        <Box sx={{ mb: { xs: 5, md: 6 } }}>
            <Typography
                variant="h4"
                sx={{
            ...
            >
                {members.map((member, index) => (
                    <Box
                        key={index}
                        sx={{
                            flex: isBig ? "1 1 400px" : "1 1 250px",
                            maxWidth: isBig ? 450 : 300,
                            p: isBig ? 5 : 4,
                            borderRadius: 4,
                            background: "#fff",
    ...
    return (
        ...
        >
            <Box sx={{ maxWidth: 1200, mx: "auto" }}>
                {renderSection("Meet Our Facilitator", facilitator, true)}
                {renderSection("Meet Our Mentors", mentors)}
                {renderSection("Meet Our Advisors", advisors, true)}
            </Box>
        </Box>
    );
}
